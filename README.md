...
| 層 | 位置 | 規模(2026-07) | 是什麼 |
|---|---|---|---|
| 🗺 地圖層 | [`taxonomy/`](taxonomy/) | MSC 2020 全樹 **6,006 條**分類 | 整個數學研究版圖的座標系 |
| 📇 索引層 | [`catalog/`](catalog/) | 8 領域 **246 題** + 4 大名單 | 著名難題的百科索引,一題一列、狀態經查證 |
| 📚 深度層 | [`problems/`](problems/) | **11 題**(持續擴充) | 一題一資料夾的深度檔案:陳述、時間線、文獻、AI 協作方向 |

索引層的題目累積夠多資料後會「升格」為深度層資料夾;未來目標是把索引層的每題標上 MSC 代碼,讓三層互相串接。詳見[路線圖](ROADMAP.md)。
...
```
catalog/       索引層:未解難題總目錄,一題一列(246 題;格式見 catalog/_format.md)
problems/      深度層:一題一資料夾(格式見 problems/_template/README.md)
taxonomy/      地圖層:MSC 分類概覽 + msc/ 完整分類樹(6,006 條,授權見下)
ai/            AI 如何協助數學研究的方法論
tools/         資料工程腳本(如 tools/msc/generate.py 分類樹產生器)
ROADMAP.md     專案願景與路線圖
```
